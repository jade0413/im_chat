...
|---|---|---|
| im-web | 80 | 前端静态页 + 反代 |
| im-gateway | 8080 | WebSocket（容器内明文，TLS 前置卸载） |
...
curl -fsS http://127.0.0.1:8080/health                       # 网关
curl -fsS http://127.0.0.1:8080/metrics                      # 网关 Prometheus 指标
```
...
容器内一律明文，**TLS 在最前面的 nginx / 云 SLB 卸载**（architecture §13.8）。最小前置配置：把 `https://你的域名` 终止 TLS 后，`/` 与 `/api/` 转给 im-web:80，`/ws` 升级为 WebSocket 转给 im-gateway:8080。客户端连 `wss://你的域名/ws`。

> ⚠️ **部署前必修**：仓库内 `im-web/nginx.conf` 当前把 `/ws` 反代到 `http://im-gateway-rust:9090`，但 compose 里网关服务名是 `im-gateway`、监听端口是 `8080`。**服务名和端口都对不上**，照原样上线 Web 端 WebSocket 必然连不通。上线前改成 `proxy_pass http://im-gateway:8080;`（或在你的前置 nginx 里直接指向 `im-gateway:8080`）。详见第 7 节。
...
`3306`(MySQL)、`6379`(Redis)、`5672/15672`(RabbitMQ + 管理台)、`9000/9001`(MinIO + 控制台)、`3000/9090/3100`(观测栈)、`8081/9091`(im-server)、`8080`(网关，应走前置而非直连)。
...
| 项 | 现状 | 处理 |
|---|---|---|
| Web `/ws` 反代地址 | `im-web/nginx.conf` 指向 `im-gateway-rust:9090` | 改为 `im-gateway:8080`，否则 Web 端聊天连不上 |
...
| 客户端连不上 WS | 前置 nginx `/ws` 是否指向 `im-gateway:8080`；`ALLOWED_ORIGINS` 是否含来源域名；fd 是否打满 |
...
cp .env.example .env && vim .env          # 改密码/JWT/Origin
# 改 im-web/nginx.conf: /ws -> im-gateway:8080
