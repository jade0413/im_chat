...
3. **gRPC 寻址**：网关 `UPSTREAM_GRPC` 支持逗号分隔多地址（2026-07-02 D43，如 `http://java-1:9091,http://java-2:9091`），多地址时网关内置 tower p2c 客户端负载均衡，多 im-server 实例直接列出地址即可，无需额外 L7 LB。单地址行为不变（启动即连、fail-fast）。注意：地址列表变更需重启网关生效；若未来要动态扩缩容再考虑 DNS 服务发现或前置 envoy。
