...
| 会话状态 | 推送目标 |
|---|---|
| open（待接待） | 当前 tenant 所有在线坐席（`is_agent=true` 的 user） |
...
| `conversation.agent_id BIGINT NULL` | `conversation` 表 | 新增，认领后写入 |
| 新增 `visitor_profile` 表 | 新增 | 见 §2.2 |

Flyway 迁移：`V6__cs.sql`
...
| 值 | 含义 | 行为 |
|---|---|---|
| 0 | offline | 不接收 CS 推送，widget 显示"离线" |
| 1 | online | 正常接收推送，可认领会话 |
| 2 | busy | 接收推送，但 widget 可显示"忙碌中" |
...
T33 推送路由扩展：open 会话推送时，只推 `agent_status IN (1,2)` 的在线坐席，offline 坐席不推（但消息仍存库，上线后可从 inbox 看到）。

### 11.4 Widget 展示

- 有任一坐席 online/busy → 显示"我们在线，发消息给我们"
