# 实时音视频通话设计（D45/D50，2026-07-03 修订）

> 范围：**1v1 语音/视频 + 小群 mesh 通话 MVP**。媒体 = WebRTC P2P/mesh + 自建 coturn 中继；信令 = 现有 WS 帧通道（网关 D19 透传，Rust 零改动）。群通话不是会议系统，不接 SFU，默认最多 9 人。
...
Flutter A <═══════════ WebRTC SRTP 音视频（P2P/小群 mesh，失败走 coturn UDP 中继）═══════════> Flutter B
```
...
- 群通话按小群 mesh 实现：主叫只占用自己，成员真正接听后才占线；老成员向新接入成员发 offer，避免同时 offer glare；主叫挂断即整场结束，这是 MVP 简化语义。
...
- `call:{tenantId}:{callId}` HASH：caller_id / callee_id / group_id / invited_ids / active_ids / media / state / client_call_id / invite_at / answer_at，TTL：INVITING 90s → ACTIVE 4h（每次 SIGNAL 不续期，4h 为单通话硬上限）
- `call_user:{tenantId}:{userId}` STRING=callId：占线标记，TTL 与主键同步；1v1 INVITE 时对主叫、被叫都 SETNX；群通话 INVITE 只锁主叫，成员 accept 成功才锁自己
...
状态迁移用 Lua 保证原子（1v1 多端同时接听只有一个赢；群通话成员可陆续加入，accept 后服务端重读 session 快照再通知已接通成员，避免并发接听漏建连）。
