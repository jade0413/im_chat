...
@Service
public class CallSignalHandler implements CmdHandler {

  private final CallService callService;

  public CallSignalHandler(CallService callService) {
    this.callService = callService;
  }

  @Override
  public int cmd() {
    return Cmd.CALL_SIGNAL_VALUE;
  }
  ...
  public byte[] handle(ConnCtx ctx, byte[] body) {
    try {
      return callService.signal(ctx, CallSignal.parseFrom(body)).toByteArray();
    } catch (InvalidProtocolBufferException ex) {
      throw new ImException(ErrorCode.VALIDATION_FAILED, "invalid CallSignal body", ex);
    }
  }
}
