...
@Service
public class CallHangupHandler implements CmdHandler {

  private final CallService callService;

  public CallHangupHandler(CallService callService) {
    this.callService = callService;
  }

  @Override
  public int cmd() {
    return Cmd.CALL_HANGUP_VALUE;
  }
  ...
  public byte[] handle(ConnCtx ctx, byte[] body) {
    try {
      return callService.hangup(ctx, CallHangup.parseFrom(body)).toByteArray();
    } catch (InvalidProtocolBufferException ex) {
      throw new ImException(ErrorCode.VALIDATION_FAILED, "invalid CallHangup body", ex);
    }
  }
}
