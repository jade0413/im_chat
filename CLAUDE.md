...
| D45 | 2026-07-02 | **1v1 实时语音通话 MVP**（Jade 拍板三项：实时通话 / WebRTC P2P+自建 coturn / 仅 1v1 语音，详见 docs/call-service-design.md）：媒体 P2P+TURN 中继（服务端不碰媒体），信令走现有 WS 帧（D19 网关零改动）。proto：frame.proto 新增 CALL_INVITE(40)/ANSWER(41)/SIGNAL(42)/HANGUP(43)/CALL_NOTIFY(45)/CALL_ACK(49)，新建 body/call.proto（SDP/ICE 不透明透传、client_call_id 幂等、media 预留 VIDEO），error.proto 新增 7xxx 通话段。**协议 §3 修订：CALL_NOTIFY 是 MSG_PUSH 之外唯一 need_ack=true 帧**（振铃丢失不可接受；D44 客户端泛化 ack 保证前向安全）。Java：新模块 im-call-service——Redis 状态机（INVITING→ACTIVE→删除，Lua 原子 CAS 多端先接者赢；busy/idem/deadline 键同生共死）、4 个 CmdHandler、TURN REST 凭证（HMAC-SHA1，与 coturn use-auth-secret 共享密钥）、超时 sweeper（全局 deadline ZSET+原子 claim，多实例安全，与 OutboxPoller 同构）、call_record CDR（V13 迁移）。规则：忙线服务端代答不打扰被叫；全端振铃先接者赢其余端 ANSWERED_ELSEWHERE；30s 超时；被叫全端离线不直接失败，主叫保持呼叫态等 timeout（MVP 无离线推送，被叫离线期间不响铃）。Flutter：flutter_webrtc + data/call/CallEngine（信令+PC 状态机，trickle ICE 候选缓存，主叫收 ACCEPTED 才 createOffer）+ features/call/CallPage + LumoApp 全局来电路由 + 聊天页 C2C 拨打入口 + 四端麦克风权限。部署：compose 新增 coturn（host 网络，UDP 3478+49160-49200）。⚠️ 待：proto 三端重生成（tool/generate_proto.sh + maven + cargo）、全量编译测试、双人真机联调；沙箱无工具链未验证。二阶段挂 Open Questions：离线被叫推送唤醒/CallKit、通话记录气泡入会话、断连联动挂断、视频/群通话（SFU） | 客服/社交 IM 的标配能力；P2P+coturn 是 D24 自装路线下零第三方成本的最短路径；信令复用 WS 使网关与推送链路零新增组件 |

| D46 | 2026-07-03 | **钱包/红包/转账**（Jade 拍板：资金形态=平台虚拟币 COIN 充值单向不承诺提现；钱包地址=平台内部类 TRC20 base58check 地址不上链，非好友凭地址转账收手续费）：新模块 **im-wallet-service**，复式记账（wallet_account/wallet_txn/wallet_ledger，账务按真实资金标准），金额一律 BIGINT 分禁浮点，余额只允许条件 UPDATE，系统科目（FEE/RP_ESCROW/TRANSFER_ESCROW/MERCHANT/RECHARGE）去余额化防热点，幂等 idem_key 唯一键兜底。红包：DB 行锁内二倍均值实时计算、uk(packet_id,user_id) 防重、24h 过期 sweeper 退回；转账：好友=确认收款制（PENDING→RECEIVED/REFUNDED 24h）、地址=直接到账+手续费。content.proto 新增强类型 RedPacketContent=30/TransferContent=31（消息不可变、状态真相在表，D40 模式）；message 发送链路调 WalletRpc.VerifyMoneyMsg 防伪造资金消息；新增 MessageRpc.SendConvNotification（会话灰条通用）；error.proto 8xxx 段；支付 PIN（bcrypt，错5锁15min）。领取/收款走 REST，无新 Cmd，网关零改动。详见 docs/wallet-redpacket-design.md | 虚拟币规避支付牌照合规但账务不降级，未来升级真实资金零改动账务层；wallet 是频道付费创建（D48）与开放平台支付（D49）的底座，必须最先实现 |
| D47 | 2026-07-03 | **朋友圈**：新模块 **im-moment-service**，读扩散拉模式（好友数百+万级在线下 IN 查询足够，好友>2000 再升写扩散 timeline）；仅好友可见/互动，点赞评论按**共同好友规则**服务端过滤；不进消息体系（独立表，无 seq），红点走新 Cmd MOMENT_NOTIFY=27 best-effort + unread-hint 拉取对齐（真相在表）；moment/moment_like/moment_comment/moment_interaction 四表 + user.moment_visible_days（三天/半年可见）；审核复用 D16 管道（outbox MomentCreatedEvent → moderation → MomentRpc.RevokeMoment，moderation_log 加 biz_type 列）。开源参考结论：OpenIM 核心不含朋友圈，自研 | 规模下读扩散成本最低且无失效维护；共同好友过滤必须服务端做防关系泄露；错误码 3101~3103 |
| D48 | 2026-07-03 | **频道+全局搜索**（Jade 拍板：Telegram 式单向广播频道+付费创建）：ConvType 新增 CHANNEL=5，频道复用 conversation/message/seq 全链路（group_info 加 kind/handle/is_public/description 列），仅 owner/admin 可发言（message 链路调 GroupRpc.CheckPostPermission，缓存60s、撤权主动失效）；订阅者角色 subscriber，订阅前历史可见，上限套餐 channel_max_subscribers（默认5000）；推送分叉：订阅数≤1000 现有 MSG_PUSH 扇出，>1000 降级 CONV_NOTIFY 脏通知+拉取（D13 预判路径首次落地）。公开目录 public_directory 独立表 + MySQL8 ngram FULLTEXT（D24 不引 ES，升级路径留 binlog→ES）；用户搜索维持 D42 精确匹配不放宽；付费创建走 WalletRpc.Pay saga（pending_payment 中间态+sweeper 恢复，幂等键=group_id）。频道内禁红包/通话。归 im-group-service 不新建模块 | 频道=特化的群，复用管道零新体系；handle 复用 D42 规范；错误码 41xx |
| D49 | 2026-07-03 | **开放平台 MVP**（Jade 拍板：H5 容器+JSSDK，不做小程序运行时）：新模块 **im-open-service**。发现页九宫格 discovery_entry；OAuth2 授权码简化流（code Redis 300s GETDEL 一次性，open_id 每 app 随机映射不泄露 user_id，手机号永不下发）；JSSDK bridge（flutter_inappwebview 单通道）：getAuthCode/pay/share/close/getEnv，**每次调用校验当前 URL ∈ app domain_whitelist**，收银台金额只从服务端拉绝不信 H5；支付：第三方 HMAC-SHA256 签名统一下单（secret 派生 sign_key 落库，bcrypt secret 不可逆仍可签名）→ 原生收银台 PIN → WalletRpc.Pay 入 MERCHANT 科目 → 回调重试5次+主动查单兜底；分享卡片走 CustomContent("open.app.card")（D20 逃生口正确用例，WS 协议零改动）。⚠️ /open/api/* 无 JWT，TenantContext 由 app_id 反查手动装载（拦截器盲区，P0 审查项）。错误码 9xxx | 公众号网页生态模式是第三方接入成本最低且容器安全边界可控的形态；商户结算/出金列 Open Question |
| D50 | 2026-07-03 | **通话 MVP 修订：1v1 音视频 + 小群 mesh 通话**（追认 2026-07-03 未提交审计结论）：D45 的 WebRTC P2P/coturn/WS 信令不变，media=VIDEO 正式启用；群通话只做小群 mesh，不接 SFU，`im.call.group-call-max-members` 默认 9 且代码限制 2~9，超过直接拒绝，避免 500 人群推送风暴和 N² 媒体面爆炸。群 INVITE 只锁主叫，成员 accept 成功才占线；accept 后重读 Redis 最新 active_ids 再通知已接通成员，防并发同时接听漏建连；群 reject 只停本账号其他端铃，不结束整场；主叫挂断=整场结束（MVP 简化，写入 docs/call-service-design.md）；客户端 incoming 30s 本地超时兜底；普通全端离线不直接代答，主叫等待 timeout。正式会议能力（SFU、主持人、主叫退出后继续、禁麦等）二阶段独立设计 | 小规模社交群聊通话先满足可用性，避免为了 MVP 引入 SFU/会议系统复杂度；审计指出的 DoS 面必须先用硬上限收住 |
| D51 | 2026-07-08 | **网关运行期审查修复七项**（docs/reviews/2026-07-08-gateway-runtime-review.md，Jade 拍板当日实施）：**P0-1** read_loop 增加 close 信号 select（biased）——修复僵尸读循环：被 KICK/半死链/drain 的连接在客户端拒不关 TCP 时曾可无限存活并继续以旧 ConnCtx 上行 Dispatch，绕过 D11 互踢与 D27 token 失效语义；**P0-2** `ws.max_message_size/max_frame_size(max_frame_bytes)` 传输层限帧（tungstenite 默认 64MiB，旧 64KB 检查在整条消息缓冲完之后才生效）；**P1-3** uplink 指标 cmd 白名单，未知归并 `cmd="other"` 防基数攻击；**P2-4** 新增 client_ip 模块（trusted CIDR + XFF 右起第一个不可信地址）+ nginx /ws 补 XFF/X-Real-IP + compose `IM_GATEWAY_TRUSTED_PROXY_CIDRS`——修复 Web 端经反代时 per-IP 限流退化为全局 20/s；**P2-5** 停机封顶宽限（最坏 ≈ 2×drain_timeout）；**P2-6** AUTH 阶段跳过 WS Ping/Pong（共享 deadline 防拖延）；**P2-7** 连接级信号量（32）限 OnPushAcked/RefreshRoute 在途转发防 ACK 洪泛。另：新增全局连接数上限 `IM_GATEWAY_MAX_CONNECTIONS`（默认 5 万，握手 503+capacity 计数）；冗余清理（ConnKey derive、FrameSendResult/PushSendResult 合并、重复 cmd 检查删除、Config::for_test 共用）。`cargo test` 25/25 零警告（本次顺带闭环 D43 遗留的 axum 0.8 工具链验证）。记录在案未处理：TLS 缺位（Flutter 直连 8082 明文，违 D15，挂 Open Question）、dispatch 超时须 < 心跳容忍度、观测直方图二阶段 | 僵尸读循环是踢线/token 失效语义的实际破口，必须修；其余为万级在线目标下的攻击面收敛与运维确定性 |
...
- docs/call-service-design.md（实时音视频与小群 mesh 通话设计，2026-07-03 修订，关联 D45/D50）
- docs/wallet-redpacket-design.md（钱包/红包/转账，2026-07-03 确定，待实现，关联 D46，**是 D48/D49 的底座须最先实现**）
- docs/moments-design.md（朋友圈，2026-07-03 确定，待实现，关联 D47）
- docs/channel-discovery-design.md（频道+全局搜索，2026-07-03 确定，待实现，关联 D48，依赖 D46）
- docs/open-platform-design.md（开放平台 H5 容器+JSSDK+支付，2026-07-03 确定，待实现，关联 D49，依赖 D46）
...
- [ ] 【D51】生产 WS/API 的 TLS 终止方案（前置 LB/nginx 统一终止 vs 网关自挂 rustls）——当前 compose 下 Flutter 直连 8082 明文 WS，与 D15"全链路 TLS"矛盾，上生产前必须定；per-IP 限流的 trusted proxy 机制已就绪（P2-4）
...
- [ ] 【D46】真实充值渠道选型与提现合规（形态若升级真实资金）；PIN 找回（实名/短信依赖）与免密小额；红包高吞吐 Redis 预拆分路径
- [ ] 【D46/D49】MERCHANT 商户科目结算/出金语义（虚拟币形态下商户余额如何消化）
- [ ] 【D47】朋友圈分组可见/不看他/不让他看（visibility 枚举+scope 表语义已预留）；陌生人可见 N 条；写扩散 timeline 升级阈值
- [ ] 【D48】付费订阅频道（订阅费分账）；频道评论区；搜索 ES 升级阈值与拼音/同义词；订阅数>5000 读扩散
- [ ] 【D49】开发者自助后台与 app 审核流；URL 签名（wx.config 式）；模板消息；小程序运行时（重决策独立评估）
