...
| D50 | 2026-07-03 | **通话 MVP 修订：1v1 音视频 + 小群 mesh 通话**（追认 2026-07-03 未提交审计结论）：D45 的 WebRTC P2P/coturn/WS 信令不变，media=VIDEO 正式启用；群通话只做小群 mesh，不接 SFU，`im.call.group-call-max-members` 默认 9 且代码限制 2~9，超过直接拒绝，避免 500 人群推送风暴和 N² 媒体面爆炸。群 INVITE 只锁主叫，成员 accept 成功才占线；accept 后重读 Redis 最新 active_ids 再通知已接通成员，防并发同时接听漏建连；群 reject 只停本账号其他端铃，不结束整场；主叫挂断=整场结束（MVP 简化，写入 docs/call-service-design.md）；客户端 incoming 60s 本地超时兜底。正式会议能力（SFU、主持人、主叫退出后继续、禁麦等）二阶段独立设计 | 小规模社交群聊通话先满足可用性，避免为了 MVP 引入 SFU/会议系统复杂度；审计指出的 DoS 面必须先用硬上限收住 |
